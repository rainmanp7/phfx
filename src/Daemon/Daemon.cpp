...
void renameDataDir() {
  std::string phfireDir = Tools::getDefaultDataDirectory();
  boost::filesystem::path phfireDirPath(phfireDir);
  if (boost::filesystem::exists(phfireDirPath)) {
    return;
  }

  std::string dataDirPrefix = phfireDir.substr(0, phfireDir.size() + 1 - sizeof(CRYPTONOTE_NAME));

  boost::filesystem::path darkNoteDirPath(dataDirPrefix + "darknote");
  if (boost::filesystem::exists(darkNoteDirPath)) {
    boost::filesystem::rename(darkNoteDirPath, phfireDirPath);
  } else {
    ...
    if (boost::filesystem::exists(boost::filesystem::path(duckNoteDirPath))) {
      boost::filesystem::rename(duckNoteDirPath, phfireDirPath);
    }
  }
}
